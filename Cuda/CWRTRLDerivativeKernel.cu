...
extern "C"
{
		...
		)
	{
		...
		if (partialId < D_HIDDEN_UNITS * D_HIDDEN_UNITS * D_INPUT_UNITS && (contextByActivations || simulationStep % periods[unitId] == 0))
		{
		...
		)
	{
		...
		if (partialId < D_HIDDEN_UNITS * D_HIDDEN_UNITS * D_HIDDEN_UNITS && (contextByActivations || simulationStep % periods[unitId] == 0))
		{
